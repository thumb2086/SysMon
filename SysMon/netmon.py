...
def set_autostart(enable):
    try:
        ...
        if enable:
            exe = os.path.abspath(sys.argv[0]) if getattr(sys, "frozen", False) else os.path.abspath(__file__)
...
class SysMon:
    ...
    def __init__(self):
        ...
        self._daily = load_daily()
        self._today = fmt_date()
        if self._today not in self._daily:
            self._daily[self._today] = {"dn": 0, "up": 0}
        self._daily_dn_baseline = 0
        self._daily_up_baseline = 0
    ...
    def _bg_daily_log(self):
        def work():
            while True:
                time.sleep(300)
                today = fmt_date()
                if today not in self._daily:
                    self._daily[today] = {"dn": 0, "up": 0}
    ...
    def _bar(self, canvas, pct, fg, tag="fill"):
        ...
        h = int(canvas["height"])
        items = canvas.find_withtag("fill_bg")
        if not items:
            canvas.create_rectangle(0, 0, w, h, fill=self.BG, outline="", tags="fill_bg")
            bw = max(0, int(w * min(pct, 100) / 100))
            canvas.create_rectangle(0, 0, bw, h, fill=fg, outline="", tags=(tag, "fill_bar"))
        else:
            bw = max(0, int(w * min(pct, 100) / 100))
            bars = canvas.find_withtag("fill_bar")
            if bars:
                canvas.coords(bars[0], 0, 0, bw, h)
                canvas.itemconfig(bars[0], fill=fg)
    ...
    def _draw_chart(self, canvas, data, color):
        ...
        if not hasattr(canvas, '_line_id') or canvas._line_id is None:
            canvas.delete("all")
            canvas._line_id = canvas.create_line(coords, fill=color, width=1.5, smooth=False)
            canvas._base_id = canvas.create_line(0, gh - 1, gw, gh - 1, fill=self.DIM, dash=(2, 4))
        else:
            canvas.coords(canvas._line_id, *coords)
            canvas.coords(canvas._base_id, 0, gh - 1, gw, gh - 1)
    ...
    def _build(self):
        ...
        self._auto_btn = tk.Label(self._hdr, text="", bg=self.CARD,
                                  font=("Segoe UI", 8), cursor="hand2")
        self._auto_btn.pack(side=tk.RIGHT, fill=tk.Y, padx=(0, 2))
        self._auto_btn.bind("<Button-1>", self._toggle_autostart)
        self._upd_auto_btn()
        ...
        close_lb = tk.Label(self._hdr, text="⎯  ", bg=self.CARD, fg=self.DIM,
                            font=("Segoe UI", 9), cursor="hand2")
        close_lb.pack(side=tk.RIGHT, fill=tk.Y)
        close_lb.bind("<Button-1>", lambda e: self._minimize_to_tray())
    ...
    def _upd_auto_btn(self):
        txt = "● AUTO" if self._autostart_enabled else "○ AUTO"
        fg = self.GRN if self._autostart_enabled else self.DIM
        self._auto_btn.config(text=txt, fg=fg)

    def _toggle_autostart(self, e=None):
    ...
    def _upd_cpu(self):
        ...
        pct = sum(cores) / len(cores) if cores else 0
        f = psutil.cpu_freq()
    ...
    def _upd_net(self):
        ...
        if len(dn_data) > 1:
            ...
            for h_data, color, tag in [(dn_data, self.GRN, '_ng_dn'), (up_data, self.ORG, '_ng_up')]:
                pts = []
                for i, v in enumerate(h_data):
                    x = i * step
                    y = gh - 2 - (v / mx_all) * (gh - 4)
                    pts.append((x, y))
                coords = [coord for p in pts for coord in p]
                if not hasattr(self, tag) or getattr(self, tag) is None:
                    setattr(self, tag, self._ng.create_line(coords, fill=color, width=1.5, smooth=False))
                else:
                    self._ng.coords(getattr(self, tag), *coords)

        if not hasattr(self, '_ng_base') or self._ng_base is None:
            self._ng_base = self._ng.create_line(0, gh // 2, gw, gh // 2, fill=self.DIM, dash=(2, 4))
        else:
            self._ng.coords(self._ng_base, 0, gh // 2, gw, gh // 2)
    ...
    def _rebuild_disk(self):
        ...
        for dr in self._dsk_part_rows:
            try:
                ...
                self._bg_part_usage.append((dr["mp"], u))
                dr["txt"].config(text=f"{u.used / 2**30:.0f}/{u.total / 2**30:.0f}G")
            except Exception:
                self._bg_part_usage.append((dr["mp"], None))
    ...
    def _upd_disk(self):
        ...
        if mx > 0 and len(self._dr_h) > 1:
            ...
            if not hasattr(self, '_dg_bars'):
                ...
                for i in range(len(self._dr_h)):
                    ...
                    r1 = self._dg.create_rectangle(x, gh - 2 - rh, x + bw // 2, gh - 2, fill=self.PPL, outline="")
                    r2 = self._dg.create_rectangle(x + bw // 2, gh - 2 - wh, x + bw, gh - 2, fill=self.CYN, outline="")
                    self._dg_bars.append((r1, r2))
            else:
                for i in range(min(len(self._dr_h), len(self._dg_bars))):
                    x = i * (bw + 1)
                    rh = (self._dr_h[i] / mx) * (gh - 4)
                    wh = (self._dw_h[i] / mx) * (gh - 4)
                    r1, r2 = self._dg_bars[i]
                    self._dg.coords(r1, x, gh - 2 - rh, x + bw // 2, gh - 2)
                    self._dg.coords(r2, x + bw // 2, gh - 2 - wh, x + bw, gh - 2)
        ...
        self._part_tick += 1
        if self._part_tick % 10 == 1:
            self._bg_part_usage = []
            for dr in self._dsk_part_rows:
                try:
                    self._bg_part_usage.append((dr["mp"], psutil.disk_usage(dr["mp"])))
                except Exception:
                    self._bg_part_usage.append((dr["mp"], None))

        for dr, (mp, u) in zip(self._dsk_part_rows, self._bg_part_usage):
            if u is None:
                continue
            p = u.percent
